...
export const RAW_KNOWLEDGE_POINTS: TaxonomyKnowledgePoint[] = [
  ...
  },
  {
    ...
    keyFormulaOrTip:
      "当 r1, r2 均小于 10% 时，r1 × r2 极小可直接略去先算代数和。",
    baseAccuracy: 72,
    prerequisites: ["d_growth_rate"],
    nextSteps: [],
  ...
  },
  {
    ...
    examWeight: "高频必考 (20%)",
    description:
      "图形按固定步长（45°/90°/135°）顺时针/逆时针旋转或沿网格平移轨迹。",
    keyFormulaOrTip:
  ...
  },
  {
    ...
    keyFormulaOrTip:
      "画出对称轴方向（横/竖/斜45°），观察对称轴是否顺时针步进旋转。",
    baseAccuracy: 80,
    prerequisites: [],
...
export const EXTRA_RELATIONS = [
  {
    source: "d_growth_amount",
    target: "d_proportion_analysis",
    type: "prerequisite",
    label: "速算赋能",
  },
  {
    source: "d_base_current",
    target: "d_mixed_growth",
    type: "prerequisite",
    label: "基期支撑",
  },
  {
    source: "g_overlay_subtraction",
    target: "g_black_white_matrix",
    type: "prerequisite",
    label: "相消同构",
  },
  {
    source: "g_step_rotation",
    target: "g_3d_spatial_folding",
    type: "prerequisite",
    label: "旋转支撑",
  },
  {
  ...
  {
    source: "d_chart_reading",
    target: "v_detail_verify",
    type: "cross_domain",
    label: "细节敏锐度迁移",
  },
  {
    source: "g_count_elements",
    target: "d_growth_rate",
    type: "cross_domain",
    label: "定量思维协同",
  },
];
 ...
 */
export function findKnowledgePointForQuestion(
  q: Question,
): TaxonomyKnowledgePoint {
  const categoryPool = RAW_KNOWLEDGE_POINTS.filter(
  ...
  // 2. Stem / explanation text matching (e.g. 图形题规律关键词)
  {
    ...
    for (const point of categoryPool) {
      if (
        point.subCategoryKeywords.some(
          (kw) => kw.length >= 3 && textToCheck.includes(kw),
        ) ||
...
import type { UserAnswerRecord, StudyStats } from "../types";
...
export interface PointStudyStats {
  ...
  /** 考点对应题库的真实 subCategory 集合（按题量降序，首选为专项练习跳转目标）；0 题考点为空数组 */
  subCategories: string[];
}

/** 考点在当前题库中对应的真实题目集合 */
export function questionsForPoint(
...
export function computePointStats(
  ...
  const questions = questionsForPoint(point, allQuestions);
  const ids = new Set(questions.map((q) => q.id));
  // 跳转目标取题库真实 subCategory（题量降序），修复 taxonomy 命名 ≠ 题库命名的断链
  const subCount = new Map<string, number>();
  questions.forEach((q) =>
    subCount.set(q.subCategory, (subCount.get(q.subCategory) || 0) + 1),
  );
  const subCategories = Array.from(subCount.entries())
    .sort((a, b) => b[1] - a[1])
    .map(([s]) => s);
