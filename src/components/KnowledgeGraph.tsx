...
export const KnowledgeGraph: React.FC<KnowledgeGraphProps> = ({
  ...
  const linksData: KnowledgeLink[] = useMemo(() => {
    ...
    // Root to categories
    links.push(
      { source: 'root_hub', target: 'cat_verbal', type: 'hierarchy', strength: 1.0 },
      { source: 'root_hub', target: 'cat_data', type: 'hierarchy', strength: 1.0 },
      { source: 'root_hub', target: 'cat_graphic', type: 'hierarchy', strength: 1.0 }
    );
    ...
    const weakNodeIds = new Set(nodesData.filter((n) => n.status === 'weak').map((n) => n.id));
    nodesData.forEach((node) => {
  ...
  useEffect(() => {
    if (!selectedNode) {
      const firstWeak = nodesData.find((n) => n.status === 'weak' && n.type === 'topic');
      setSelectedNode(firstWeak || nodesData.find((n) => n.id === 'g_overlay_subtraction') || nodesData[1]);
    }
  ...
  useEffect(() => {
    ...
    const glowFilter = defs.append('filter').attr('id', 'glow-weak').attr('x', '-30%').attr('y', '-30%').attr('width', '160%').attr('height', '160%');
    ...
    const filteredNodes = nodesData.filter((node) => {
      ...
      if (filterMode === 'weakness') return node.type === 'root' || node.type === 'category' || node.status === 'weak';
      if (filterMode === 'verbal') return node.type === 'root' || node.category === 'verbal';
      if (filterMode === 'data') return node.type === 'root' || node.category === 'data';
      if (filterMode === 'graphic') return node.type === 'root' || node.category === 'graphic';
      return true;
    });
    ...
    const filteredLinks = linksData
      .filter((link) => {
        ...
        if (filterMode === 'weakness' && link.type !== 'weakness_warning' && link.type !== 'hierarchy') {
          return false;
        }
        return true;
      })
    ...
    const simulation = d3
      ...
      .force(
        ...
        d3
          ...
          .id((d) => d.id)
          .distance((d) => {
            if (d.type === 'hierarchy') return d.source.id === 'root_hub' ? 140 : 85;
      ...
      .force(
        ...
        d3
          ...
          .strength((d) => (d.type === 'root' ? -450 : d.type === 'category' ? -280 : -140))
      )
      .force('center', d3.forceCenter(width / 2, height / 2))
      .force(
        'collide',
        d3.forceCollide<KnowledgeNode>().radius((d) => (d.radius || 20) + 16).iterations(3)
      )
      .alphaDecay(0.028);
    ...
    const link = linkGroup
      ...
      .attr('stroke-width', (d) => (d.type === 'weakness_warning' ? 2.5 : d.type === 'prerequisite' ? 1.8 : 1.2))
      ...
      .attr('stroke-opacity', (d) => (d.type === 'weakness_warning' ? 0.9 : 0.6))
      .attr('marker-end', (d) => {
    ...
    node
      ...
      .attr('filter', (d) => (d.status === 'weak' ? 'url(#glow-weak)' : 'url(#node-shadow)'));
    ...
    node
      .append('text')
      .text((d) => (d.type === 'root' ? '北森全景' : d.shortName))
      .attr('text-anchor', 'middle')
      .attr('dy', (d) => (d.type === 'topic' ? '-0.1em' : '0.35em'))
      .attr('font-size', (d) => (d.type === 'root' ? '11px' : d.type === 'category' ? '11px' : '9.5px'))
    ...
    node
      .on('mouseenter', (event, d) => {
        ...
        // Dim non-neighbors
        node.transition().duration(150).style('opacity', (n) => (neighborIds.has(n.id) ? 1 : 0.22));
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-[#786c5e] mt-0.5">
                完整覆盖 25+ 核心考点 · 掌握度颜色编码 · 红色虚线代表跨考点薄弱警示连线
              </p>
            </div>
          </div>
        </div>
      ...
      <div className={`px-5 sm:px-6 pb-6 grid grid-cols-1 lg:grid-cols-3 gap-5 ${isFullscreen ? 'flex-1' : ''}`}>
        ...
        <div
          ...
          className={`lg:col-span-2 relative bg-[#26201a] rounded-2xl border border-[#4a3e31] shadow-inner overflow-hidden flex flex-col ${
            isFullscreen ? 'min-h-[620px] h-[calc(100vh-250px)]' : 'min-h-[480px] sm:min-h-[540px]'
          }`}
        >
          ...
          <div className="absolute top-3 right-3 z-10 flex flex-col gap-1 bg-[#3a3026]/90 backdrop-blur-xs p-1 rounded-xl border border-[#524436] text-[#ded3bd] shadow-md">
            ...
            <button
              ...
              title={isFullscreen ? '退出全屏' : '全屏沉浸查看 (Toggle Fullscreen)'}
              aria-label="全屏切换"
            >
          ...
          <svg ref={svgRef} className="w-full h-full cursor-grab active:cursor-grabbing flex-1" />
        </div>
          ...
          {selectedNode ? (
            <div className="space-y-4 animate-in fade-in duration-150">
                ...
                <div className="bg-[#fffdfa] p-2.5 rounded-xl border border-[#ded3bd]">
                  ...
                  <div
                    className={`font-bold text-sm mt-0.5 ${
                      selectedNode.mistakesCount > 0 ? 'text-[#b91c1c]' : 'text-[#15803d]'
                    }`}
                  >
              ...
              <div className="text-xs text-[#5c4e3f] leading-relaxed bg-[#fffdfa] p-3 rounded-xl border border-[#ded3bd]">
                <span className="font-semibold text-[#26201a]">📌 考点定义与命题规律：</span>
            ...
            <div className="py-16 text-center text-[#8c7e6d] space-y-2">
              <Compass className="w-8 h-8 mx-auto text-[#b45309] animate-spin" style={{ animationDuration: '8s' }} />
              <p className="text-xs">点击左侧图谱中的任意节点，查看深度掌握分析与秒杀技巧</p>
            </div>
          )}

          {/* Action button: Jump to Practice Mode with this category */}
          {selectedNode && selectedNode.type === 'topic' && onSelectSubCategory && (
            <button
              onClick={() => onSelectSubCategory(selectedNode.category, selectedNode.shortName)}
            ...
            >
              <span>前往【{selectedNode.shortName}】专项刷题</span>
