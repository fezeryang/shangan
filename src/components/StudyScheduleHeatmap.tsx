...
const TIME_PERIOD_CONFIG: Record<
  string,
  { name: string; period: HourlyData['period']; defaultProductivity: number; defaultAccuracy: number; task: string }
> = {
  0: { name: '深夜疲劳期', period: 'lateNight', defaultProductivity: 35, defaultAccuracy: 58, task: '建议休息或仅复盘温习' },
  1: { name: '深夜休眠期', period: 'lateNight', defaultProductivity: 25, defaultAccuracy: 50, task: '大脑疲惫，禁止刷难题' },
  2: { name: '深夜休眠期', period: 'lateNight', defaultProductivity: 20, defaultAccuracy: 45, task: '充足睡眠是考前关键' },
  3: { name: '深夜休眠期', period: 'lateNight', defaultProductivity: 20, defaultAccuracy: 45, task: '睡眠恢复' },
  4: { name: '凌晨苏醒期', period: 'night', defaultProductivity: 30, defaultAccuracy: 55, task: '晨起轻量回顾公式' },
  5: { name: '清晨预热期', period: 'night', defaultProductivity: 45, defaultAccuracy: 65, task: '常识记忆与公式默写' },
  6: { name: '晨起唤醒期', period: 'morning', defaultProductivity: 65, defaultAccuracy: 75, task: '言语理解快速热身' },
  7: { name: '晨间黄金期', period: 'morning', defaultProductivity: 80, defaultAccuracy: 82, task: '言语阅读 & 资料速算' },
  8: { name: '上午专注期', period: 'morning', defaultProductivity: 88, defaultAccuracy: 86, task: '全真模考 & 图形推理' },
  9: { name: '上午心流爆发期', period: 'morning', defaultProductivity: 95, defaultAccuracy: 92, task: '重难点攻坚：复杂图推与资料分析' },
  10: { name: '上午心流爆发期', period: 'morning', defaultProductivity: 96, defaultAccuracy: 93, task: '真题高强度冲刺与限时压测' },
  11: { name: '上午高效收尾', period: 'morning', defaultProductivity: 85, defaultAccuracy: 84, task: '短平快单项突破' },
  12: { name: '午间生理恢复', period: 'noon', defaultProductivity: 55, defaultAccuracy: 68, task: '午休补充精力，避免复杂计算' },
  13: { name: '午后倦怠期', period: 'noon', defaultProductivity: 48, defaultAccuracy: 62, task: '做题易粗心，建议错题复盘' },
  14: { name: '下午能量回升', period: 'afternoon', defaultProductivity: 75, defaultAccuracy: 78, task: '言语细节推断与逻辑填空' },
  15: { name: '下午思维高峰', period: 'afternoon', defaultProductivity: 90, defaultAccuracy: 89, task: '资料分析图表与基期比重计算' },
  16: { name: '下午高效攻坚', period: 'afternoon', defaultProductivity: 92, defaultAccuracy: 90, task: '空间折叠与重叠相消专项' },
  17: { name: '傍晚练习期', period: 'afternoon', defaultProductivity: 78, defaultAccuracy: 80, task: '回顾全天错题本' },
  18: { name: '晚间用餐休整', period: 'evening', defaultProductivity: 60, defaultAccuracy: 72, task: '轻松翻阅速算技巧小卡片' },
  19: { name: '晚间黄金心流期', period: 'evening', defaultProductivity: 93, defaultAccuracy: 91, task: '完整套卷全真模考 (建议与北森上机同频)' },
  20: { name: '晚间黄金心流期', period: 'evening', defaultProductivity: 94, defaultAccuracy: 92, task: '限时提速挑战 & 难点攻破' },
  21: { name: '晚间巩固期', period: 'evening', defaultProductivity: 82, defaultAccuracy: 83, task: '知识图谱薄弱节点定向补强' },
  22: { name: '夜间复盘期', period: 'lateNight', defaultProductivity: 68, defaultAccuracy: 75, task: '当天错题二次重做' },
  23: { name: '睡前整理期', period: 'lateNight', defaultProductivity: 50, defaultAccuracy: 65, task: '查漏补缺，放松心态' },
};

const DAY_NAMES = ['周日', '周一', '周二', '周三', '周四', '周五', '周六'];
const TIME_BLOCKS = [
  { name: '早晨 (06:00-09:00)', range: '06:00-09:00', startHour: 6, endHour: 9 },
  { name: '上午 (09:00-12:00)', range: '09:00-12:00', startHour: 9, endHour: 12 },
  { name: '下午 (14:00-18:00)', range: '14:00-18:00', startHour: 14, endHour: 18 },
  { name: '晚间 (19:00-22:00)', range: '19:00-22:00', startHour: 19, endHour: 22 },
];
...
export const StudyScheduleHeatmap: React.FC<StudyScheduleHeatmapProps> = ({
  ...
  const hourlyStats: HourlyData[] = useMemo(() => {
    const hourBuckets: Record<number, { total: number; correct: number; totalTime: number }> = {};
    for (let h = 0; h < 24; h++) {
      hourBuckets[h] = { total: 0, correct: 0, totalTime: 0 };
    }
    ...
    answerRecords.forEach((rec) => {
      try {
        ...
        if (h >= 0 && h < 24) {
          ...
          hourBuckets[h].totalTime += rec.timeSpentSec || 30;
        }
      } catch (e) {
        // fallback
      }
  ...
  // 2. Process Week Matrix
  const weekMatrix: WeekMatrixCell[][] = useMemo(() => {
    const matrix: WeekMatrixCell[][] = [];
    const recordsMap: Record<string, { total: number; correct: number }> = {};
    answerRecords.forEach((rec) => {
      try {
        const date = new Date(rec.answeredAt);
        ...
        if (bIdx !== -1) {
          const key = `${day}-${bIdx}`;
          if (!recordsMap[key]) recordsMap[key] = { total: 0, correct: 0 };
  ...
  const getCellBgColor = (score: number, mode: HeatmapMode, isSelected: boolean) => {
    if (mode === 'productivity' || mode === 'accuracy') {
      if (score >= 90) return isSelected ? 'bg-[#24683a] ring-2 ring-[#15803d] text-white' : 'bg-[#2e7d47] hover:bg-[#24683a] text-white';
      if (score >= 80) return isSelected ? 'bg-[#edf7ee] ring-2 ring-[#24683a] text-[#14532d] border border-[#a3d9ae]' : 'bg-[#edf7ee] hover:bg-[#dcfce7] text-[#15803d] border border-[#bbf7d0]';
      if (score >= 70) return isSelected ? 'bg-[#fef7ea] ring-2 ring-[#b45309] text-[#78350f] border border-[#ebdcb9]' : 'bg-[#fef7ea] hover:bg-[#faeed6] text-[#854d0e] border border-[#ebdcb9]';
      if (score >= 55) return isSelected ? 'bg-[#fff4ea] ring-2 ring-[#c2410c] text-[#9a3412] border border-[#f4d7b8]' : 'bg-[#fff4ea] hover:bg-[#fee7d6] text-[#9a3412] border border-[#f4d7b8]';
      return isSelected ? 'bg-[#fef2f2] ring-2 ring-[#b91c1c] text-[#991b1b] border border-[#fca5a5]' : 'bg-[#fef2f2] hover:bg-[#fee2e2] text-[#991b1b] border border-[#fecaca]';
    } else {
      // Volume mode
      if (score > 10) return isSelected ? 'bg-[#b45309] ring-2 ring-[#9a3412] text-white' : 'bg-[#b45309] text-white';
      if (score > 5) return isSelected ? 'bg-[#d97706] ring-2 ring-[#b45309] text-white' : 'bg-[#d97706] text-white';
      if (score > 0) return isSelected ? 'bg-[#fef7ea] ring-2 ring-[#b45309] text-[#854d0e] border border-[#ebdcb9]' : 'bg-[#fef7ea] text-[#854d0e] border border-[#ebdcb9]';
  ...
  return (
    <div className="bg-[#fdfbf7] rounded-2xl p-6 border border-[#e3d9c4] shadow-2xs space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-[#786c5e] mt-0.5">
                追踪 24 小时生物节律与刷题正确率变化，避开易错疲劳陷阱，锁定个人高效冲刺黄金窗口
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-xl bg-[#fef7ea] border border-[#ebdcb9]">
          <div className="flex items-center justify-between text-xs font-semibold text-[#78350f]">
            <span className="flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-[#b45309]" />
              北森上机考位对齐策略
            </span>
            <span className="px-2 py-0.5 rounded-md bg-[#fef3c7] text-[#92400e] text-[10px] font-bold">
              生物钟校准
            </span>
          </div>
          <div className="mt-2 text-xl font-extrabold text-[#78350f] font-display">
            上午 10:00 模拟上机
          </div>
          <div className="mt-1 text-xs text-[#854d0e] leading-relaxed">
            大厂笔试多安排在上午10:00或下午15:00，提前 3 天将做题兴奋度锁定在此时间段。
          </div>
        </div>
      </div>
      ...
      {viewType === 'hourly' ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between text-xs text-[#786c5e] font-medium px-1">
            <span>24小时全天候能效节律（点击任意时段方块查看详细指标与推荐任务）：</span>
            <div className="flex items-center gap-3 text-[11px]">
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded bg-[#2e7d47] inline-block" /> 极佳 (≥90)
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded bg-[#edf7ee] border border-[#bbf7d0] inline-block" /> 良好 (80-89)
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded bg-[#fef7ea] border border-[#ebdcb9] inline-block" /> 平稳 (70-79)
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded bg-[#fff4ea] border border-[#f4d7b8] inline-block" /> 波动 (55-69)
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded bg-[#fef2f2] border border-[#fecaca] inline-block" /> 疲劳 (&lt;55)
              </span>
            </div>
          </div>
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="text-lg font-bold font-display text-white">
                    {currentDetail.label} - {(currentDetail.hour + 1).toString().padStart(2, '0')}:00
                  </span>
                ...
                <p className="text-xs text-[#ded3be] flex items-center gap-1">
                  ...
                  推荐安排：<span className="text-[#fef08a] font-semibold">{currentDetail.recommendedTasks}</span>
                </p>
              </div>
              ...
              <div className="flex items-center gap-4 text-xs">
                ...
                <div className="bg-white/10 px-3 py-2 rounded-xl text-center">
                  ...
                  <div className="text-base font-extrabold text-[#fde047] font-display">
                    {currentDetail.totalAnswered} <span className="text-[10px] text-[#ded3be]">题</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between text-xs text-[#786c5e] font-medium px-1">
            ...
            <span className="text-[11px] text-[#8c7e6d]">基于近 7 天做题轨迹加权计算</span>
          </div>
            ...
            <table className="w-full text-xs text-left border-collapse">
              <thead>
                <tr className="border-b border-[#e8ded0] text-[#786c5e]">
                  ...
                  {TIME_BLOCKS.map((tb) => (
                    <th key={tb.name} className="py-2.5 px-3 font-semibold text-center">
                      <div>{tb.name}</div>
                      <div className="text-[10px] text-[#8c7e6d] font-normal">{tb.range}</div>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[#ede4d2]">
                {weekMatrix.map((row, dayIdx) => (
                  <tr key={dayIdx} className="hover:bg-[#f8f3e8]/60 transition-colors">
      ...
      <div className="pt-2">
        <h4 className="font-bold text-[#26201a] text-sm mb-3 flex items-center gap-2">
          ...
          <span>基于做题生理节律的【北森高分全天冲刺排程表】</span>
        </h4>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          <div className="p-3.5 rounded-xl border border-[#ebdcb9] bg-[#fef7ea]/60 hover:bg-[#fef7ea] transition-colors space-y-1.5">
            ...
            <p className="text-[#854d0e] text-[11px] leading-relaxed">
              <strong>适宜科目：</strong>言语逻辑、选词填空与高频错题回顾。<br />
          ...
          <div className="p-3.5 rounded-xl border border-[#bbf7d0] bg-[#edf7ee]/60 hover:bg-[#edf7ee] transition-colors space-y-1.5">
            ...
            <p className="text-[#15803d] text-[11px] leading-relaxed">
              <strong>适宜科目：</strong>图形推理 3D 折叠/空间截面 + 全真模拟考试。<br />
              <strong>目标：</strong>大脑认知与注意力峰值期，严卡 50 秒配速冲刺。
            </p>
          </div>
          ...
          <div className="p-3.5 rounded-xl border border-[#ded2bd] bg-[#f8f3e8]/60 hover:bg-[#f8f3e8] transition-colors space-y-1.5">
            ...
            <p className="text-[#6b3b1f] text-[11px] leading-relaxed">
              <strong>适宜科目：</strong>资料分析两期比重、增长量百化分速算。<br />
          ...
          <div className="p-3.5 rounded-xl border border-[#e3d9c4] bg-[#fcf8ee]/60 hover:bg-[#fcf8ee] transition-colors space-y-1.5">
            ...
            <p className="text-[#5c4e3f] text-[11px] leading-relaxed">
              <strong>适宜科目：</strong>知识图谱薄弱节点定向拔高 + 错题本重做清零。<br />
