...
import { drawablyHighlight } from "drawably";
...
/** 秒杀口诀标签：手绘标记笔高亮（本页招牌笔迹） */
const ShortcutLabel: React.FC = () => {
  const ref = useSketch<HTMLSpanElement>((el) => drawablyHighlight(el), []);
  return (
    <span ref={ref} className="font-bold text-[#92400e]">
      ⚡ 秒杀口诀：
    </span>
  );
};
...
export const FormulaGuide: React.FC = () => {
  ...
  const basePeriod =
    rDecimal === -1 ? "0" : (numXianQi / (1 + rDecimal)).toFixed(2);
  const growthAmount =
    rDecimal === -1
      ? "0"
      : ((numXianQi * rDecimal) / (1 + rDecimal)).toFixed(2);

  // Two-period proportion difference
  const numA = parseFloat(calcA) || 0;
  const numB = parseFloat(calcB) || 0;
  const proportionTrend =
    numA > numB ? "上升 ↑" : numA < numB ? "下降 ↓" : "持平 ＝";
    ...
    <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-4 bg-[#f8f3e8] rounded-xl border border-[#e3d8c2] space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <label className="block text-[#6e6153] mb-1">
                  现期量 (万元/件/人):
                </label>
                <DrawablyInput
                  type="number"
                  ...
                  className="w-full !px-2.5 !py-1.5 bg-[var(--card)] text-[color:var(--ink)] font-mono"
                />
              </div>
              <div>
                <label className="block text-[#6e6153] mb-1">
            ...
            <div className="p-3 bg-[#edf7ee] rounded-lg border border-[#bbf7d0] text-xs flex justify-between items-center">
              <div>
                ...
                <strong className="text-[#14532d] font-display text-base">
                  {basePeriod}
                </strong>
              </div>
              <div>
          ...
          <div className="p-4 bg-[#f8f3e8] rounded-xl border border-[#e3d8c2] space-y-3">
            ...
            <div className="p-3 bg-[#fef7ea] rounded-lg border border-[#ebdcb9] text-xs flex justify-between items-center">
              <div>
                ...
                <strong className="text-[#78350f] font-bold text-sm">
                  {proportionTrend}
                </strong>
              </div>
              <div>
                <span className="text-[#854d0e] font-medium">差值上限: </span>
                <strong className="text-[#78350f] font-mono text-sm">
                  &lt; {maxDiff} 个百分点
                </strong>
              </div>
            </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <DrawablyInput
          ...
          className="w-full sm:w-64 !py-2 text-xs bg-[var(--card)] text-[color:var(--ink)]"
        />
      </div>
          ...
          return (
            ...
            >
              <div className="space-y-2.5">
                ...
                <h4 className="font-bold text-[#26201a] text-sm sm:text-base">
                  {item.title}
                </h4>
                ...
                <p className="text-xs text-[#5c4e3f] leading-relaxed">
                  {item.description}
                </p>
