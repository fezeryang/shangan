...
const results = await page.evaluate(() => {
  const out = [];
  const assert = (name, ok, detail) => out.push(`${ok ? 'PASS' : 'FAIL'} ${name}${ok ? '' : ' — ' + detail}`);

  // 1. Structure: no nested <pre> (react-markdown v10 fix), coffee-dark code block
  const pres = document.querySelectorAll('pre');
  assert('no nested <pre>', document.querySelectorAll('pre pre').length === 0, 'pre>pre found');
  assert(
  ...
  assert('body 15px on sm+', bs2 && bs2.fontSize === '15px', bs2 && bs2.fontSize);
  assert('leading-relaxed 1.625', bs2 && parseFloat(bs2.lineHeight) / parseFloat(bs2.fontSize) > 1.55, bs2 && bs2.lineHeight);
  assert('warm ink #3b3227', bs2 && bs2.color === 'rgb(59, 50, 39)', bs2 && bs2.color);
  ...
  assert('ol marker amber #b45309', marker === 'rgb(180, 83, 9)', String(marker));
  ...
  assert('strong wash visible on #f8f3e8 (Δ 40–120)', delta > 40 && delta < 120, `blended Δ=${delta.toFixed(0)}`);
  ...
  assert(
    'h2 amber tick',
    h2b && h2b.backgroundColor === 'rgba(180, 83, 9, 0.8)' && h2b.width === '3px',
  ...
  if (h2) {
    ...
    assert('h2 mt > mb', parseFloat(h2s.marginTop) > parseFloat(h2s.marginBottom), `${h2s.marginTop} vs ${h2s.marginBottom}`);
  }
  ...
  const chatFirst = document.querySelector('[data-case="chat"] .markdown-body > *');
  assert(
  ...
  if (h1 && h2 && h3) {
    ...
    assert('heading ramp 16/17/16-base steps', s1 > s2 && s2 >= s3, `${s1}/${s2}/${s3}`);
  }

  // 13. no horizontal overflow anywhere (CJK break-words + scroll containers)
  const overflow = [...document.querySelectorAll('.markdown-body, [data-case]')].filter(
    (el) => el.scrollWidth > el.clientWidth + 1
  );
  assert('no horizontal overflow', overflow.length === 0, overflow.map((e) => e.className).join(','));
...
const mres = await mobile.evaluate(() => {
  ...
  assert('table wrapper scrolls internally', tblWrap && tblWrap.scrollWidth >= tblWrap.clientWidth, 'no wrapper');
  return out;
});
